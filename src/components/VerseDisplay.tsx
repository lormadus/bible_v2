...
import { VerseSuggestion } from '../types.ts'; 
...
export const VerseDisplay: React.FC<VerseDisplayProps> = ({ suggestion }) => {
  ...
  const handleToggleSpeech = useCallback(() => {
    if (!speechSynthesisSupported || !utterance || typeof window === 'undefined' || !('speechSynthesis' in window)) return;
  ...
  return (
    <div className="mt-8 p-6 bg-sky-50 border-l-4 border-sky-500 rounded-r-lg shadow-md animate-fadeIn">
      ...
      {speechSynthesisSupported && (
       ...
       {!speechSynthesisSupported && utterance === null && typeof window !== 'undefined' && ( 
...
// This IIFE for dynamic CSS injection should be safe as it checks for document/window.
// However, with Vite, CSS can be imported directly into JS/TS files or as global CSS.
// For simplicity, keeping it, but a Vite-idiomatic way would be to move this to a CSS file.
(function addFadeInAnimation() {
  ...
  // Check if @keyframes rule exists
  for (let i = 0; i < document.styleSheets.length; i++) {
    try {
      const sheet = document.styleSheets[i];
      // Ensure rules can be accessed (CORS)
      if (!sheet.cssRules && !sheet.rules) continue; 
      const rules = sheet.cssRules || sheet.rules;
      if (!rules) continue;
      for (let j = 0; j < rules.length; j++) {
        const rule = rules[j];
        if (typeof CSSKeyframesRule !== 'undefined' && rule instanceof CSSKeyframesRule && rule.name === animationName) {
  ...
  if (!animationExists) {
    ...
    if (styleSheet) {
      try {
        ...
        // Check if .animate-fadeIn class rule exists
        for (let i = 0; i < document.styleSheets.length; i++) {
            try {
                const sheet = document.styleSheets[i];
                if (!sheet.cssRules && !sheet.rules) continue;
                ...
                for (let j = 0; j < rules.length; j++) {
                    const rule = rules[j];
                     if (typeof CSSStyleRule !== 'undefined' && rule instanceof CSSStyleRule && rule.selectorText === '.animate-fadeIn') {
