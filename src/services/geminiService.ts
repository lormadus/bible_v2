...
if (!apiKey && typeof window !== 'undefined') { // Check typeof window to prevent warning during server-side build
  console.warn("VITE_API_KEY environment variable is not set. API calls will fail on the client.");
}
...
export const fetchVerseSuggestion = async (selections: UserSelections): Promise<VerseSuggestion> => {
  ...
  try {
    ...
    // Corrected regex: removed unnecessary backslashes before \s and \n.
    // \`\`\` is the correct way to match literal backticks in a regex literal.
    ...
    if (error instanceof Error) {
        if (error.message.toLowerCase().includes("api key not valid") || error.message.toLowerCase().includes("invalid api key")) {
             throw new Error("제공된 API 키가 유효하지 않습니다. .env 파일 및 설정을 확인해주세요.");
        }
